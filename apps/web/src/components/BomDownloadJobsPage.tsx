...
import { ArrowLeft, HardDriveDownload, Loader2, RefreshCcw, XCircle } from 'lucide-react';
import {
...
type StatusFilter = 'all' | BomDownloadJobStatus;
...
export const BomDownloadJobsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      void fetchBomDownloadJobsForUser({
        batchId: batchIdFilter || null,
        status: statusFilter,
        limit: 100,
      }).then((js) => {
  ...
  const handleCancel = async (jobId: string) => {
    ...
    try {
      ...
      if (!ok) alert('无法取消：任务可能已开始执行或已结束。');
      await load();
    } catch (e) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h2 className="text-2xl font-bold text-slate-900 mt-1">下载任务</h2>
            <p className="text-slate-500 mt-1 text-sm">网页触发的 it-Artifactory 拉取队列（由 bom-scanner-worker 执行）。</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex flex-wrap gap-3 items-end">
        <div>
          <label className="block text-xs font-medium text-slate-600 mb-1">批次</label>
          <select
            value={batchIdFilter}
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-3 py-2 text-left text-xs font-semibold text-slate-700">批次</th>
